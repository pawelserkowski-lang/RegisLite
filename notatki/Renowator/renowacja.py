...
def detect_rotation_strict(image_path):
    ...
    Zadajemy pytanie: "Gdzie jest góra głów?"
    """
    ...
    try:
        # Używamy 800px, żeby model wyraźnie widział twarze
        with Image.open(image_path) as img:
        ...
        if response and response.status_code == 200:
            ...
            # Konwersja Kierunek Głów -> Ile obrócić w LEWO (Counter-Clockwise - domyślne PIL)
            # PIL .rotate(90) obraca w lewo (przeciwnie do zegara)
            
            if direction == "UP": return 0      # Jest OK
            if direction == "DOWN": return 180  # Obrót o 180
            if direction == "RIGHT": return 90  # Głowy w prawo -> Obróć 90 w lewo -> OK
            if direction == "LEFT": return 270  # Głowy w lewo -> Obróć 270 w lewo (to to samo co 90 w prawo) -> OK
...
def detect_exact_corners(image_path):
    """
    Detect 4 corners logic. (Tym razem nie musimy podawać kąta, bo obraz jest już obracany fizycznie przed tą funkcją)
    """
    clean_model = MODEL_DETECTION.replace("models/", "")
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{clean_model}:generateContent"
    headers = {"Content-Type": "application/json", "x-goog-api-key": api_key}
...
def process_single_file(file_path, filename):
    ...
    try:
        ...
        # Sprawdza: Gdzie ludzie mają głowy?
        angle_to_fix = detect_rotation_strict(file_path)
        
        if angle_to_fix != 0:
            img_rot = Image.open(file_path)
            # PIL .rotate obraca w lewo. Jeśli głowy są w PRAWO, .rotate(90) postawi je do pionu.
            # Jeśli głowy w LEWO (jak na Twoich skanach), angle=270 -> .rotate(270) = .rotate(-90) w prawo.
        ...
        for idx, spath in enumerate(processed_parts):
            ...
            if os.path.exists(final_path):
                results_log.append(f"⏩ Istnieje: {final_name}")
                continue

            print(f"🚀 [{filename}] Przetwarzanie... ")
            success = restore_generative_final(spath, final_path)
            
            if success:
                results_log.append(f"✅ Gotowe: {final_name}")
            else:
                results_log.append(f"❌ Błąd API: {filename}")

        # Czyste sprzątanie po wątku
...
def main():
    ...
    print(f"⚡ SUPER-BATCH START (20 Wątków). Semantyczna Rotacja (Góra/Lewo). Plików: {len(files)}")
