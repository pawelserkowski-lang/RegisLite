...
async def start_debug_loop(session_id: str):
    ...
    for i in range(1, MAX_ROUNDS + 1):
        ...
        for f in all_files:
            ...
            has_tag = any(
                tag in content for tag in ["FIXME", "TODO", "BUG", "ERROR"]
            )
            ...
            if has_tag or was_edited or i == 1:
                # W 1. rundzie bierzemy wszystko, w kolejnych tylko istotne
        ...
        # Jeśli kontekst jest pusty, ale kod działa -> OK
        if not context_files:
            # Fallback: weź main.py lub app.py żeby sprawdzić czy działa
            context_files = [
                f for f in all_files
                if "main" in f["path"] or "app" in f["path"]
            ]

        log(f"📉 Zoptymalizowany kontekst: {len(context_files)} plików")
