...
# Apply nest_asyncio to allow nested event loops (asyncio.run inside pytest)
nest_asyncio.apply()
...
def test_tool_executor_sh(mock_session_manager):
    """
    Sync test wrapper that uses asyncio.run() explicitly.
    This avoids pytest-asyncio's Runner conflicts when nest_asyncio is used.
    """
    async def _test():
...
def test_tool_executor_ai(mock_session_manager):
    """
    Sync test wrapper for AI tool.
    """
    async def _test():
        executor = ToolExecutor(mock_session_manager)
